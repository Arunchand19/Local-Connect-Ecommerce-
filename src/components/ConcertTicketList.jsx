...
const ConcertTicketList = () => {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        ...
        console.error('Error fetching concert tickets:', err);
  ...
  if (tickets.length === 0) {
    return <div className="no-tickets-container">No concert tickets available at the moment.</div>;
  }
            ...
            {ticket.ticketImage && ticket.ticketImage.data && (
              <div className="ticket-image-container">
                <img
                  ...
                  alt={`Ticket for ${ticket.performerName}`}
                  className="ticket-image"
                />
              </div>
            )}
            <div className="ticket-info">
              <h3>{ticket.performerName}</h3>
...
export default ConcertTicketList; 